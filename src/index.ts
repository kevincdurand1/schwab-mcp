  ...
  async init() {
    ...
    this.server.tool(
      ...
      async (_args: {}, context: any) => {
        ...
        if (!accessToken) {
          console.error('[getSchwabAccounts] Error: Access token not available in this.props.')
          return {
    ...
    this.server.tool(
      ...
      async (_args: {}, context: any) => {
        ...
        if (!accessToken) {
          console.error('[getAccountNumbers] Error: Access token not available in this.props.')
          return {
            content: [{ type: 'text', text: 'Error: Access token not available. Authentication may be required.' }],
          }
        }
    ...
    this.server.tool(
      ...
      async (_args: {}, context: any) => {
        ...
        if (!accessToken) {
          console.error('[getOrders] Error: Access token not available in this.props.')
          return {
            content: [{ type: 'text', text: 'Error: Access token not available. Authentication may be required.' }],
          }
        }
        ...
        try {
          ...
          if (orders.length === 0) {
            return { content: [{ type: 'text', text: 'No Schwab orders found.' }] }
          }
          ...
          return {
            content: [{ type: 'text', text: `An error occurred fetching Schwab orders: ${error.message}` }],
          }
        }
      },
    )
    ...
    this.server.tool(
      ...
      async (_args: {}, context: any) => {
        ...
        if (!accessToken) {
          console.error('[getSchwabTransactions] Error: Access token not available in this.props.')
          return {
            content: [{ type: 'text', text: 'Error: Access token not available. Authentication may be required.' }],
          }
        }
        ...
        try {
          ...
          const transactions = await trader.transactions.getTransactions(accessToken)

          if (transactions.length === 0) {
            return { content: [{ type: 'text', text: 'No Schwab transactions found.' }] }
          }
          ...
          return {
            content: [
              { type: 'text', text: 'Successfully fetched Schwab transactions:' },
              // Stringify JSON data and return as text, based on SDK examples
              { type: 'text', text: JSON.stringify(transactionSummaries, null, 2) },
            ],
          }
        } catch (error: any) {
