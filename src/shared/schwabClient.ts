import { createApiClient, EnhancedTokenManager, type SchwabApiClient } from '@sudowealth/schwab-api';
import { type RedisClientType } from 'redis';

// Use console.error for logging to avoid polluting stdout (MCP communication channel)
const log = {
  info: (msg: string, data?: any) => console.error(`[SchwabClient] ${msg}`, data || ''),
  error: (msg: string, data?: any) => console.error(`[SchwabClient] ${msg}`, data || ''),
};
 ...
 */
export async function getSchwabClient(redisClient: RedisClientType): Promise<SchwabApiClient> {
    ...
    save: async (newTokenData) => {
      ...
      try {
        ...
        const updatedTokenData = {
          ...
          tokenType: (newTokenData as any).tokenType || existing.tokenType || 'Bearer',
          scope: (newTokenData as any).scope || existing.scope || 'api',
