...
export async function registerTools(server: Server, tokenStore: TokenStore): Promise<void> {
  // Combine all tools
        ...
        try {
          ...
          if (refreshResponse.ok) {
            ...
            if (updatedTokenData && new Date(updatedTokenData.expiresAt) > new Date()) {
              ...
              const redisClient = (tokenStore as any).client;
              ...
              const result = await toolSpec.call(client, toolArgs as any);
