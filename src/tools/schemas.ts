...
const requestedDestination = z.enum(['INET', 'ECN_ARCA', 'CBOE', 'AMEX', 'PHLX', 'ISE', 'BOX', 'NYSE', 'NASDAQ', 'BATS', 'C2', 'AUTO'])
const stopPriceLinkBasis = z.enum(['MANUAL', 'BASE', 'TRIGGER', 'LAST', 'BID', 'ASK', 'ASK_BID', 'MARK', 'AVERAGE'])
...
const priceLinkBasis = z.enum(['MANUAL', 'BASE', 'TRIGGER', 'LAST', 'BID', 'ASK', 'ASK_BID', 'MARK', 'AVERAGE'])
const priceLinkType = z.enum(['VALUE', 'PERCENT', 'TICK'])
const taxLotMethod = z.enum(['FIFO', 'LIFO', 'HIGH_COST', 'LOW_COST', 'AVERAGE_COST', 'SPECIFIC_LOT', 'LOSS_HARVESTER'])
const specialInstruction = z.enum(['ALL_OR_NONE', 'DO_NOT_REDUCE', 'ALL_OR_NONE_DO_NOT_REDUCE'])
const orderStrategyType = z.enum(['SINGLE', 'CANCEL', 'RECALL', 'PAIR', 'FLATTEN', 'TWO_DAY_SWAP', 'BLAST_ALL', 'OCO', 'TRIGGER'])
const status = z.enum([
...
const amountIndicator = z.enum(['DOLLARS', 'SHARES', 'ALL_SHARES', 'PERCENTAGE', 'UNKNOWN'])
...
const OrderStrategy = z.object({
  accountNumber: z.string(),
  advancedOrderType: z.enum(['NONE', 'OTO', 'OCO', 'OTOCO', 'OT2OCO', 'OT3OCO', 'BLAST_ALL', 'OTA', 'PAIR']),
...
const TransactionMutualFund = TransactionBaseInstrument.extend({
  ...
  type: z.enum(['NOT_APPLICABLE', 'OPEN_END_NON_TAXABLE', 'OPEN_END_TAXABLE', 'NO_LOAD_NON_TAXABLE', 'NO_LOAD_TAXABLE', 'UNKNOWN']),
...
const AccountAPIOptionDeliverable = z.object({
  ...
  apiCurrencyType: z.enum(['USD', 'CAD', 'EUR', 'JPY']), // Removed optional
...
const UserDetails = z.object({
  cdDomainId: z.string(),
  login: z.string(),
  type: z.enum(['ADVISOR_USER', 'BROKER_USER', 'CLIENT_USER', 'SYSTEM_USER', 'UNKNOWN']),
