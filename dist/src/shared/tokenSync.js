...
const log = {
    info: (msg, data) => console.error(`[TokenSync] ${msg}`, data || ''),
    error: (msg, data) => console.error(`[TokenSync] ${msg}`, data || ''),
};
/**
 * Token synchronization utility to ensure all services have access to the latest tokens
 */
export class TokenSyncManager {
    static TOKEN_KEY = 'schwab-mcp:token';
    static TOKEN_SYNC_KEY = 'schwab-mcp:token-sync';
    static TOKEN_TTL = 31 * 24 * 60 * 60; // 31 days
    redisClient = null;
    async connect() {
    ...
    async storeTokens(tokenData) {
        ...
        try {
            ...
            await this.redisClient.setEx(TokenSyncManager.TOKEN_SYNC_KEY, TokenSyncManager.TOKEN_TTL, JSON.stringify({
     ...
     */
    async needsRefresh(thresholdMinutes = 5) {
    ...
    async deleteTokens() {
        ...
        try {
            ...
            await this.redisClient.setEx(TokenSyncManager.TOKEN_SYNC_KEY, 3600, // 1 hour TTL for deletion notification
            JSON.stringify({
                timestamp: new Date().toISOString(),
                action: 'deleted'
            }));
...
//# sourceMappingURL=tokenSync.js.map