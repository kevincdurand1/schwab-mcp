import { createApiClient, sanitizeKeyForLog, sanitizeError, SchwabAuthError, SchwabApiError, } from '@sudowealth/schwab-api';
...
import { AuthErrors, formatAuthError, createJsonErrorResponse, } from './errors';
...
app.get('/authorize', async (c) => {
    try {
        ...
        if (!clientId) {
            ...
            return c.json(jsonResponse, errorInfo.status);
        }
        // If client ID is already approved, redirect directly to Schwab
        if (await clientIdAlreadyApproved(c.req.raw, oauthReqInfo.clientId, config.COOKIE_ENCRYPTION_KEY)) {
...
app.post('/authorize', async (c) => {
    try {
        ...
        if (!authRequestForSchwab?.clientId || !authRequestForSchwab?.scope) {
            ...
            oauthLogger.error(errorInfo.message);
            const jsonResponse = createJsonErrorResponse(error, undefined, errorInfo.details);
            return c.json(jsonResponse, errorInfo.status);
        }
...
app.get('/callback', async (c) => {
    try {
        ...
        const clientIdFromState = extractClientIdFromState(decodedStateAsAuthRequest);
        // Validate required AuthRequest fields directly on `decodedStateAsAuthRequest`
        if (!decodedStateAsAuthRequest?.clientId || // Should be redundant due to extractClientIdFromState
            !decodedStateAsAuthRequest?.redirectUri ||
            !decodedStateAsAuthRequest?.scope) {
            const error = new AuthErrors.InvalidState();
            const errorInfo = formatAuthError(error, {
                detail: 'Decoded state object from Schwab callback is missing required AuthRequest fields (clientId, redirectUri, or scope).',
            });
            oauthLogger.error(errorInfo.message);
            const jsonResponse = createJsonErrorResponse(error, undefined, errorInfo.details);
            return c.json(jsonResponse, errorInfo.status);
        ...
        oauthLogger.info('Exchanging authorization code for tokens with state parameter for PKCE');
        try {
            ...
            oauthLogger.error('Token exchange failed', {
                ...
                message: exchangeError instanceof Error
        ...
        try {
            ...
            oauthLogger.error('Failed to fetch user preferences', {
                ...
                message: preferencesError instanceof Error
                    ? preferencesError.message
                    : String(preferencesError),
            });
            throw new AuthErrors.NoUserId();
        }
        const userIdFromSchwab = userPreferences?.streamerInfo?.[0]?.schwabClientCorrelId;
        if (!userIdFromSchwab) {
        ...
        try {
            ...
            oauthLogger.warn('Token migration failed, continuing with authorization', {
        ...
        let mcpError = new AuthErrors.AuthCallback(); // Default MCP error for this handler
        ...
        if (isSchwabAuthError) {
            const schwabAuthErr = error;
            const errorMapping = mapSchwabError(schwabAuthErr.code, schwabAuthErr.message, schwabAuthErr.status);
